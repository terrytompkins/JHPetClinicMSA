...
describe('Vet e2e test', () => {
  ...
  const vetSample = { firstName: 'Dayana', lastName: 'Johnson' };
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/pet/api/vets+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/pet/api/vets').as('postEntityRequest');
    cy.intercept('DELETE', '/services/pet/api/vets/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (vet) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/pet/api/vets/${vet.id}`,
      }).then(() => {
        vet = undefined;
      });
    }
  ...
  describe('Vet page', () => {
    ...
    describe('with existing value', () => {
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/services/pet/api/vets?page=0&size=20>; rel="last",<http://localhost/services/pet/api/vets?page=0&size=20>; rel="first"',
              },
              body: [vet],
            }
      ...
      it('last delete button click should delete instance of Vet', () => {
        cy.intercept('GET', '/services/pet/api/vets/*').as('dialogDeleteRequest');
  ...
  describe('new Vet page', () => {
    ...
    it('should create an instance of Vet', () => {
      cy.get(`[data-cy="firstName"]`).type('Virgil').should('have.value', 'Virgil');

      cy.get(`[data-cy="lastName"]`).type('Leuschke').should('have.value', 'Leuschke');
