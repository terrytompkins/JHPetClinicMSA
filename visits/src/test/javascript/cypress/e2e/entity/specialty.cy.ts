...
describe('Specialty e2e test', () => {
  ...
  const specialtySample = { name: 'cutting-edge cross-media silver' };
  ...
  beforeEach(() => {
    cy.intercept('GET', '/services/pet/api/specialties+(?*|)').as('entitiesRequest');
    cy.intercept('POST', '/services/pet/api/specialties').as('postEntityRequest');
    cy.intercept('DELETE', '/services/pet/api/specialties/*').as('deleteEntityRequest');
  });
  ...
  afterEach(() => {
    if (specialty) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/pet/api/specialties/${specialty.id}`,
      }).then(() => {
        specialty = undefined;
      });
    }
  ...
  describe('Specialty page', () => {
    ...
    describe('with existing value', () => {
      beforeEach(() => {
        cy.authenticatedRequest({
          method: 'POST',
          url: '/services/pet/api/specialties',
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/services/pet/api/specialties?page=0&size=20>; rel="last",<http://localhost/services/pet/api/specialties?page=0&size=20>; rel="first"',
              },
              body: [specialty],
            }
      ...
      it('last delete button click should delete instance of Specialty', () => {
        cy.intercept('GET', '/services/pet/api/specialties/*').as('dialogDeleteRequest');
