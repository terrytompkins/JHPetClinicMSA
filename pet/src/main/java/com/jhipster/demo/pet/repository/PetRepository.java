...
public interface PetRepository extends JpaRepository<Pet, Long> {
    ...
    @Query(
        value = "select distinct pet from Pet pet left join fetch pet.species left join fetch pet.owner",
        countQuery = "select count(distinct pet) from Pet pet"
    )
    Page<Pet> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct pet from Pet pet left join fetch pet.species left join fetch pet.owner")
    List<Pet> findAllWithToOneRelationships();

    @Query("select pet from Pet pet left join fetch pet.species left join fetch pet.owner where pet.id =:id")
